...
export const FullScreenScrollFX = forwardRef<HTMLDivElement, FullScreenFXProps>(
  (
    ...
    // Store refs in a way that survives renders properly
    ...
    // Helper to collect word refs reliably
    const setWordRef = (el: HTMLSpanElement | null, sIdx: number, wIdx: number) => {
      if (!wordRefs.current[sIdx]) wordRefs.current[sIdx] = [];
      if (el) wordRefs.current[sIdx][wIdx] = el;
    };

    // Split words for center title
    const renderTitle = (text: string | ReactNode, sIdx: number) => {
    ...
    useLayoutEffect(() => {
      ...
      const st = ScrollTrigger.create({
        ...
        onUpdate: (self) => {
          ...
          // Clamp target to valid index range
          const target = Math.min(total - 1, Math.floor(prog * total));
          
          if (target !== lastIndexRef.current && !isAnimatingRef.current) {
             // Only auto-snap if it's a direct neighbor change to avoid jumping multiple sections at once 
             // during fast scroll unless intended.
             const diff = Math.abs(target - lastIndexRef.current);
             if (diff === 1) {
                goTo(target, false);
             }
          }
              ...
              <div className="fx-grid">
                ...
                <div className="fx-content">
                  ...
                  <div className="fx-center">
                    {sections.map((s, sIdx) => {
                      return (
                        <div key={`C-${s.id ?? sIdx}`} className="fx-featured">
